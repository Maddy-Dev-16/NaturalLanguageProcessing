...
def good_turing_smoothing_with_regression(ngram_model, vocab_size, N):
    ...
    # Now till max_count if some keys are not there then add them with frequency by regression
    for i in range(1, max_count):
        if i not in count_of_counts:
            log_N_i = b + a * np.log(i)
            N_i = np.exp(log_N_i)
            count_of_counts[i] = N_i

    for c in sorted(count_of_counts.keys()):
